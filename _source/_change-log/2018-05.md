...
Note that [`state` in the main request body](/docs/api/resources/oauth2#request-parameters) already allows these characters. <!-- OKTA-91165 -->

#### Custom Scopes in Metadata Endpoints

You can specify whether or not to include custom scopes in the metadata endpoints for [OAuth 2.0](/docs/api/resources/oauth2#retrieve-authorization-server-metadata) and [OpenID Connect](/docs/api/resources/oauth2#retrieve-authorization-server-openid-connect-metadata).
Existing custom scopes are not exposed by default. Set the [`metadataPublish` attribute to `ALL_CLIENTS`](/docs/api/resources/oauth2#scopes-properties) to change the behavior. <!-- OKTA-106548 -->
...
Version 2.6 of [the Okta Sign-In Widget](https://www.npmjs.com/package/@okta/okta-signin-widget) is available. Check out the new features and bug fixes!
