...
| [User Consent for OAuth 2.0 and OpenID Connect Flows in Early Availability](#user-consent-for-oauth-20-and-openid-connect-flows-in-early-availability) | February 28, 2018               | March 5, 2018 |
...
| [Changes to Error Messages from User Creation Requests](#changes-to-error-messages-from-user-creation-requests)  | February 28, 2018               | March 5, 2018 |
...
### User Consent for OAuth 2.0 and OpenID Connect Flows in Early Availability
...
2. Create an app via the Apps API with the appropriate values for `tos_uri`, `policy_uri`, and `consent_method`. ([Details](/docs/api/resources/apps#settings-7)) 
    ...
    Note: You can also configure an existing app in the Admin user interface: **Applications > [Application Name] > General > User Consent**.
...
### Changes to Error Messages from User Creation Requests  

Responses to an API user creation request (`/api/v1/users`) may contain error messages. The messages have changed in two ways:

1. The response `errorCauses` now contain additional fields:
...
2. Instead of a single `errorCauses` returned per message, each policy violation is reported in a separate `errorCauses`.
...
* Resetting a password with `/api/v1/users/lifecycle/reset_password` to a recently-used password, with `strict=true`, failed to reject the recently-used password. (OKTA-158966, OKTA-159085)
