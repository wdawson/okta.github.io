...
In a future post, we'll show you https://developer.okta.com/blog/2018/09/24/reactive-apis-with-spring-webflux[how to develop a Reactive API with Spring WebFlux] and friends from the Spring ecosystem. Then we'll add a React client that's capable of handling streaming data. In the meantime, check out these interesting resources on reactive programming and Spring.
